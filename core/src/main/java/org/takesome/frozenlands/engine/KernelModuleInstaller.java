...
import org.takesome.frozenlands.engine.weather.WeatherModule;
...
public final class KernelModuleInstaller {
    ...
    public KernelModuleRuntime install(KernelWorldBootstrap.KernelWorldRuntime worldRuntime) {
        ...
        moduleRegistry.register(new SkyRuntimeModule(worldRuntime.sky()), context);
        ...
        moduleRegistry.register(new WeatherModule(), context);
        ...
        context.appStateManager().attach(new LuaEventHookState(context));
        return new KernelModuleRuntime(saveManager);
    }


    private static final class SkyRuntimeModule implements EngineModule {
        private static final String ID = "engine.sky";
        private final Sky sky;
        private final Map<String, ModuleCommand> commands = new LinkedHashMap<>();

        private SkyRuntimeModule(Sky sky) {
            this.sky = sky;
            registerCommands();
        }
