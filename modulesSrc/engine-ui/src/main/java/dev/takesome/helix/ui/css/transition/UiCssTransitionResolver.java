...
import dev.takesome.helix.ui.css.properties.transition.TransitionDelayCssProperty;
import dev.takesome.helix.ui.css.properties.transition.TransitionDurationCssProperty;
import dev.takesome.helix.ui.css.properties.transition.TransitionPropertyCssProperty;
import dev.takesome.helix.ui.css.properties.transition.TransitionTimingFunctionCssProperty;
...
public final class UiCssTransitionResolver {
    ...
    public List<UiCssTransitionDescriptor> resolveAll(UiDomElement element) {
        List<String> properties = splitList(value(element, property.name(), "all"));
        List<String> durations = splitList(value(element, duration.name(), "0ms"));
        List<String> delays = splitList(value(element, delay.name(), "0ms"));
        List<String> timingFunctions = splitList(value(element, timingFunction.name(), "ease"));
    ...
    public List<UiCssTransitionDescriptor> resolveAll(Map<String, String> style) {
        List<String> properties = splitList(value(style, property.name(), "all"));
        List<String> durations = splitList(value(style, duration.name(), "0ms"));
        List<String> delays = splitList(value(style, delay.name(), "0ms"));
        List<String> timingFunctions = splitList(value(style, timingFunction.name(), "ease"));
        int count = Math.max(1, properties.size());
        ArrayList<UiCssTransitionDescriptor> out = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            out.add(new UiCssTransitionDescriptor(
